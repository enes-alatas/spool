...
func resolutionFor(t *testing.T, m *Models, name string) store.ModelResolution {
	t.Helper()
	view, err := m.View(context.Background())
	if err != nil {
		t.Fatal(err)
	}
...
func waitResolved(t *testing.T, m *Models, name string) store.ModelResolution {
	t.Helper()
	deadline := time.Now().Add(5 * time.Second)
	for time.Now().Before(deadline) {
		if res := resolutionFor(t, m, name); res.Resolved != "" {
			return res
		}
...
func TestModelsResolveTheAliasesAtStart(t *testing.T) {
	m, _ := newTestModels(t, map[string]string{"opus": "claude-opus-5-5", "fable": "claude-fable-5-1", "haiku": "claude-haiku-4-5-20251001"})
	m.Start(context.Background())

	opus := waitResolved(t, m, "opus")
	...
	waitResolved(t, m, "haiku")
	if sonnet := resolutionFor(t, m, "sonnet"); sonnet.Resolved != "" {
...
func TestModelsObserveOnlyTheDefaultRuntimesTurns(t *testing.T) {
	...
	m.Observe(ownImage, "opus", "claude-opus-4-9")
	m.Observe(bare, "opus", "claude-opus-4-9")
	m.Observe(onDefault, "claude-typed-once", "claude-typed-once")
	if res := resolutionFor(t, m, "opus"); res.Resolved != "" {
		t.Fatalf("a turn off the default runtime or image resolved opus to %q", res.Resolved)
	}

	m.Observe(onDefault, "opus", "claude-opus-5-5")
	if res := resolutionFor(t, m, "opus"); res.Resolved != "claude-opus-5-5" || res.Source != store.ResolutionTurn {
...
func TestModelsAProbeDoesNotOverwriteATurnSinceStart(t *testing.T) {
	m, db := newTestModels(t, map[string]string{"opus": "claude-opus-5-5"})
	m.Observe(&store.Loop{Runtime: store.RuntimeDocker}, "opus", "claude-opus-5-6")

	m.resolve(context.Background(), "opus")
	deadline := time.Now().Add(2 * time.Second)
	for time.Now().Before(deadline) {
		m.mu.Lock()
		running := m.inflight["opus"]
		m.mu.Unlock()
		if !running {
			break
		}
...
func TestModelsAddValidates(t *testing.T) {
	...
	for _, bad := range []string{"", "  ", "--dangerously-skip", "claude opus", string(make([]byte, maxModelLen+1))} {
		if _, err := m.Add(ctx, bad, ""); !errors.Is(err, ErrInvalidModel) {
			t.Errorf("Add(%q): err = %v, want ErrInvalidModel", bad, err)
		}
	}
	if _, err := m.Add(ctx, "opus", ""); !errors.Is(err, store.ErrDuplicate) {
		t.Errorf("Add(opus): err = %v, want ErrDuplicate, since aliases are listed", err)
	}
	entry, err := m.Add(ctx, " claude-opus-4-1 ", " pinned ")
	if err != nil {
		t.Fatal(err)
	}
	...
	if res := waitResolved(t, m, "claude-opus-4-1"); res.Source != store.ResolutionProbe {
		t.Fatalf("custom entry resolution = %+v", res)
	}
	if _, err := m.Add(ctx, "claude-opus-4-1", ""); !errors.Is(err, store.ErrDuplicate) {
