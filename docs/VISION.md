...
| Attention model | **Explicit recipients (ADR-0025).** DMs reach their loop only. Group messages remain visible to humans; native replies, mentions, and deliberate `@all` select which loops receive input, subject to lifecycle and storm rules. Unaddressed loops receive no ambient context. Opt-in follow remains deferred future work requiring a separate decision before it changes this contract. |
| Proactivity | Loops may initiate private owner DMs or separately addressed group updates, including after ticks. Destination is explicit; publicly mentioning the owner does not select a DM. Native group replies name a specific message; new loop group messages require recipients. Storm guards apply. (ADR-0025; implementation pending.) |
...
| **L1** | Workstations | Per-loop persistent Docker sandbox: `claude` runs inside, workspace/creds/tools live inside, resource limits, survive restarts, egress allowlisted. | `bypassPermissions` is safe. A loop can be pointed at a real repo without fear, and it cannot exfiltrate what it holds to arbitrary hosts. |
...
| **L3** | Slack | Chat-surface abstraction (Telegram becomes an adapter). The fleet channel lives on the hub; a surface is attached to a loop after it exists and mirrors that channel (ADR-0032). Per-loop Slack apps via generated manifests: DMs, channels, native replies, explicit mentions/broadcast, mirrors. Access model mapped to Slack workspace membership. Ambient follow is deferred for separate design. | Loops are coworkers in the team's actual Slack. |
